...
1. **Three-layer DBS** — L0 (SLOPS root) / L1 (slops-saloon/) / L2 (slops-saloon/corvus/). Layer routing rules live in each layer's `CLAUDE.md` + `AGENTS.md` wrappers, which import shared modules from `Blueprints/agent-modules/`.
2. **Corvus is free indefinitely** (decided 2026-06-15). No Y2 monetization plan. No paid tier. See `slops-saloon/corvus/Blueprints/definition-of-done.md` posture line.
...
7. **Corvus work queue** — the app's live build queue is `slops-saloon/corvus/Direction/current_sprint.md`; `slops-saloon/corvus/Direction/agent_inbox.md` is the override slot. App handoffs live under `slops-saloon/corvus/Blueprints/handoffs/`.
