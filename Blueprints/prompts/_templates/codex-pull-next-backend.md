...
After Codex finishes a Backend / Codex Phase item and you want it to autopilot to the next one without you re-drafting a phase-specific prompt. **Do not use this for Phase 1.2 backend** — that one has a hand-drafted prompt at `codex-omen-phase-1.2-sentry-backend.md` because the PII scrubber spec is too important to leave to a generic pull. From Phase 1.3+ onward, this is your default.
...
```text
Read AGENT.md first, then slops-saloon/omen/Direction/current_sprint.md.
...
5. Update `slops-saloon/omen/Blueprints/handoffs/backend-to-frontend.md` if the change affects an endpoint contract or a frontend dependency.
...
1. Open a fresh Codex session inside `SLOPS/slops-saloon/omen`.
...
- **Phase 1.2 exception.** Hand-drafted prompt lives at `_pending/codex-omen-phase-1.2-sentry-backend.md`. Fire that, not this, for Phase 1.2 backend.
...
- `_pending/codex-omen-phase-1.2-sentry-backend.md` — the one Phase 1 item that's NOT generic-pullable.
