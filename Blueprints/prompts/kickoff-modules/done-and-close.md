...
8. Commit the explicit task paths on the worktree branch with a Conventional Commit message (`feat(phase1.N): …`, `fix(…)`, `chore(…)`, `docs(…)`, `test(…)`). **Do NOT push.**
9. Confirm the task paths are clean and report what landed: file paths + commit hash + verification result + skills used/skipped + anything surprising.
