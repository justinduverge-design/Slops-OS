...
| 8 — Verify | `slops-native-sim-drive` | skill | active | **Corrected 2026-09-02, updated 2026-09-10 (0.3.0).** Registered `parked` on a macOS-host blocker that did not exist: Omen's `native-visual-evidence.yml` and `ios-ci.yml` both run on `macos-14`, and the capture pipeline was already built. The skill governs the pipeline instead of proposing one. The 2026-09-02 correction's residual claim — that local capture was blocked on a 2017 Intel MacBook Air — expired when the Mac mini became the routine iOS host, so 0.3.0 inverts the recommendation: local batch capture first, CI for parity. A blocker recorded as a fact outlived the fact twice; check the host, not the note. |
| 7 — Design QA | `slops-native-screen-design` | skill | draft | **Composes, where everything else audits.** Registered 2026-09-12 after an audit found the library had seven ways to grade a screen and none to decide one. Produces a screen contract (blocks, surface levels, scale spacing, named tokens, component resolution, badge anchors, literal copy) that `slops-native-ui-audit` then grades against. Promoted out of `draft` after its first real screen, not before. |
