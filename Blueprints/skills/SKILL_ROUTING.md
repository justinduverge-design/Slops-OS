...
| `slops-native-sim-drive` | Local capture is ordinary work and needs no assignment; a CI dispatch bills a macOS runner and stays action-gated | `Layer 0` | `draft` | **Governs Omen's two existing capture routes** rather than building a driver: `scripts/capture-screen-batch.sh` locally (both platforms, both themes, one command, free) and `native-visual-evidence.yml` in CI (macos-14, Xcode 16.2, clean-room parity). Says which route to use, when, how to add a scenario, and where output belongs. Carries the batch-gate loop — assertions inner, screenshots once at the end. **Updated 2026-09-10 (0.3.0): local is now the routine route.** The prior "only local capture is blocked (2017 MacBook Air)" note expired when the Mac mini landed. Simulator evidence never discharges a real-device gate. |
