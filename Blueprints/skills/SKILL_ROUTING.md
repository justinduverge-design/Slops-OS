...
| `slops-mobile-smoke` | Claude (plan), Codex (run) | `Layer 0` | `active` | **WEB APP ONLY (re-scoped 2026-09-02).** Automated phone-viewport smoke (SE/15 Pro/15 Pro Max) vs mobile-first-qa-playbook — touch targets, overflow, safe-area, font-size, JS errors; P0/P1/P2. Audit only; never replaces real-device QA. |
...
| `slops-intent-capture` | Claude interviews and drafts; founder reviews before commit | `Layer 0` | `draft` | Capture an idea, beta failure, or research conclusion once, in the originator's words, as a committed `intent.md` the next stage acts on. Implements the AI-Native SDLC capture-intent stage with an SLC scope gate. **Every research/review artifact must terminate in an intent, a recorded deferral, or a known-issues row** — "filed in `Direction/`" is not a terminal state. |
| `slops-founder-admin-runbook` | Claude researches and tracks; every console action founder-executed | `Layer 0` | `draft` | Dependency-ordered, stateful checklist plus first-timer walkthroughs for App Store Connect, Play Console, and provider developer portals. **Ships with no requirements encoded** — a Freshness Contract requires every row to cite current primary docs with a verification date; rows older than 90 days are unverified. Never handles credentials. |
| `slops-canvas-to-code` | Claude compiles and diffs; build agent builds | `Layer 0` | `draft` | Compile a `.dc.html` artboard into an unambiguous screen contract, emit a build prompt with a per-element acceptance checklist, then diff the built screen. Exists because "match this canvas" is not a checkable instruction, which is what burned rebuild retries. Icons require a named symbol. |
| `slops-api-hardening` | Claude scores; executor implements approved fixes | `Layer 0` | `draft` | Seven-axis scored rubric per route for what correctness tests do not cover — query efficiency, latency budget, caching, failure discipline, idempotency, error taxonomy, degradation. Evidence required per score; `UNMEASURED` is a first-class 0. Not a bug hunt — use `slops-code-review` for that. |
| `slops-provider-resilience` | Claude audits; real-account execution founder-gated | `Layer 0` | `draft` | Ten-row failure matrix per provider integration: outage, auth expiry, contract drift, empty-vs-missing, quota, partial response, boundary conditions, silent cron failure. Promotes the health-vs-data gap to an explicit step. Rows 4/5/7 are trust-critical. No credentials, no live provider calls. |
| `slops-native-sim-drive` | Local capture is ordinary work and needs no assignment; a CI dispatch bills a macOS runner and stays action-gated | `Layer 0` | `active` | **Governs Omen's two existing capture routes** rather than building a driver: `scripts/capture-screen-batch.sh` locally (both platforms, both themes, one command, free) and `native-visual-evidence.yml` in CI (macos-14, Xcode 16.2, clean-room parity). Says which route to use, when, how to add a scenario, and where output belongs. Carries the batch-gate loop — assertions inner, screenshots once at the end. **Updated 2026-09-10 (0.3.0): local is now the routine route.** The prior "only local capture is blocked (2017 MacBook Air)" note expired when the Mac mini landed. Simulator evidence never discharges a real-device gate. |
