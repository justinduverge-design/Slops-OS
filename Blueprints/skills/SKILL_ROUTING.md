...
| `slops-native-sim-drive` | Codex or Claude on a macOS/SDK host | `Layer 0` | `parked: macOS build host` | Native counterpart to the Playwright web driver — `simctl`/`xcodebuild` and `emulator`/`adb`/`gradle`, Maestro optional. Boots a simulator, installs, walks a flow, captures per-step screenshots. **Not runnable until a macOS build host exists.** Simulator evidence never discharges a real-device gate. |
