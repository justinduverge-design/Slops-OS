...
| `slops-brand-mark-forge` | Claude | `Layer 0` | `active` | End-to-end brand mark creation: founder interview → 3-4 concept directions judged at true favicon size → measured trace of the locked source (Python/PIL connected components, cubic curve fitting, registered IoU diff) → full system (monochrome, micro-mark cuts, layered Icon Composer icon) → export masters with generated filename manifest. Encodes the Omen B2 lessons: ratios not absolutes, `rescale()` not `resize()`, stroke-rendered elements are coloured by stroke, micro-marks are redrawn not scaled, never advertise an export that does not exist. Four approval gates; production asset swap is always gated. Registered 2026-07-25. |
