...
| `slops-native-sim-drive` | Per Runtime Policy + an active assignment — a dispatch bills a macOS runner and is action-gated | `Layer 0` | `draft` | **Governs Omen's existing `native-visual-evidence.yml`** (macos-14, Xcode 16.2, iPhone 16 sim, `OMEN_SCREENSHOT_SCENARIO` deterministic fixtures, per-scenario artifacts) rather than building a driver. Says when to dispatch it, how to add a scenario, and where output belongs. **Corrected 2026-09-02: previously `parked: macOS build host`, which was wrong — CI has a macOS host today; only local capture is blocked.** Simulator evidence never discharges a real-device gate. |
